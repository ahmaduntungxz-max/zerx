...
def init_language() -> None:
    ...
    if not force_change and os.path.exists(LANG_FILE):
        ...
        print()
        print(colored("  ┌─ Select Language / Pilih Bahasa", Fore.YELLOW))
        for k, v in MENU_BAHASA.items():
            print(colored(f"  │  [{k}]  {v['name']}", Fore.WHITE))
        print(colored("  └" + "─" * 28, Fore.YELLOW))
...
def log(label: str, msg: str, color: str = Fore.WHITE) -> None:
    with print_lock:
        # Visual only: slight padding for cleaner look on Termux
        print(f"  {colored(f'{label:<{LABEL_WIDTH}}', color)} {msg}")

def banner() -> None:
    """Banner visual only – tidak mengubah logika apapun"""
    w = 56
    top = "╔" + "═" * w + "╗"
    bot = "╚" + "═" * w + "╝"
    mid = "╠" + "═" * w + "╣"
    print()
    print(colored(top, Fore.CYAN))
    print(colored(r"""
  ███╗   ███╗██╗     ██╗  ██╗███████╗██████╗  ██████╗ 
  ████╗ ████║██║     ██║  ██║██╔════╝██╔══██╗██╔═══██╗
  ██╔████╔██║██║     ███████║█████╗  ██████╔╝██║   ██║
...
""", Fore.CYAN))
    ...
    print(colored("     ⚔  COMMUNITY HERO REQUEST-BL  ⚔", Fore.YELLOW))
    print(colored(f"           {CURRENT_VERSION}", Fore.GREEN))
    print(colored("        GitHub @ahmaduntungxz-max", Fore.BLUE))
    print(colored(mid, Fore.CYAN))
    print(colored("  Raw Socket • Core Affinity • NTP Precision", Fore.WHITE))
    print(colored(bot, Fore.CYAN))
    print()
...
def check_update() -> None:
    BASE          ="https://raw.githubusercontent.com/ahmaduntungxz-max/zerx/refs/heads/main/"
...
def _countdown(
    ...
    # Visual only: spinner + progress bar (logic tetap sama)
    frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
    i = 0
    while True:
        ...
        time_str = _format_remain(remain)
        # simple progress bar
        bar_len = 18
        # approximate fill (not critical)
        filled = min(bar_len, max(0, bar_len - int(remain / 1000) % (bar_len + 1)))
        bar = "█" * filled + "░" * (bar_len - filled)
        line = f"  {colored(spin, Fore.CYAN)} {colored('[Wait!]', Fore.CYAN):<{LABEL_WIDTH}} {label} {colored(time_str, Fore.YELLOW):<8} [{colored(bar, Fore.GREEN)}]"
        print(f"\r{line}  ", end="", flush=True)
        time.sleep(0.08)
        i += 1
    print("\r" + " " * 80 + "\r", end="")  # clear line
    log("[Wait!]", f"{label} → Ready!", Fore.GREEN)

# ─────────────────────── MAIN ─────────────────────── #
def main() -> None:
    banner()  # Visual only – ganti banner lama
    ...
    print(colored("  ╚══════════════════════════════════════════╝", Fore.CYAN))
    print(colored("  Terima kasih • GitHub @ahmaduntungxz-max", Fore.BLUE))
    print()
...
""", Fore.CYAN))
    print(colored("        ⚔  COMMUNITY HERO REQUEST-BL  ⚔", Fore.YELLOW))
    print(colored(f"              {CURRENT_VERSION}", Fore.WHITE))
